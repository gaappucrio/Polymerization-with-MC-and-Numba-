...
def MC_Estireno(x, xf, t, X, f, Reac, M0, ktc0MC, kd1MC, kd2MC, kfmMC, ki1MC, ki2MC, kpMC, kthMC):
  ...
  lista_M = [np.int64(x) for x in range(0)]
  lista_IB = [np.int64(x) for x in range(0)]
  while x < xf:
    ...
    if R <= Reac[0]:
      ...
      lista_t.append(t)
      lista_x.append(x)
      lista_M.append(X[3] / (6.022e23 * 1e-16))
      lista_IB.append(X[0] / (6.022e23 * 1e-16))
      if r5 > f:
        X[7] += 1
        L2.append(P2[X[5] - 1])
        P2.pop(X[5] - 1)
  ...
  return L1, L2, L3, lista_t, lista_x, lista_M, lista_IB
...
for k in range(n_loops):
    ...
    L1, L2, L3, lista_t, lista_x, lista_M, lista_IB = MC_Estireno(x, xf, t, X, f, Reac, M0, ktc0MC, kd1MC, kd2MC, kfmMC, ki1MC, ki2MC, kpMC, kthMC)
    L1_soma += L1
    L2_soma += L2
    L3_soma += L3
    #tempo_total = time.process_time() - start

total_time = time.process_time() - start
print("Tempo total de simulação: %.2f s" % (total_time))
print("Average time: %.2f s" % (total_time / n_loops))

L1_soma, L2_soma, L3_soma = lib_MC.counting_sort(L1_soma, int(max(L1_soma))), lib_MC.counting_sort(
    L2_soma, int(max(L2_soma))), lib_MC.counting_sort(L3_soma, int(max(L3_soma)))

abc, max_L = [], max(len(L1_soma), len(L2_soma), len(L3_soma))

print(max_L)
for i in range(max_L):
    abc.append(i)

for i in range(len(L1_soma), max_L):
  L1_soma.append(0)
for i in range(len(L2_soma), max_L):
  L2_soma.append(0)
for i in range(len(L3_soma), max_L):
  L3_soma.append(0)
abc = np.array(abc)

L_soma = []
for i in range(len(L1_soma)):
  L_soma.append(L1_soma[i] + L2_soma[i] + L3_soma[i])
'''
file1 = open('conversion.txt', 'w')
...
file2 = open('monomer.txt', 'w')
for i in range(len(lista_t)):
  file2.write('%.6f\t%.6f\n' % (lista_x[i], lista_M[i]))
file2.close()


file3 = open('initiator.txt', 'w')
for i in range(len(lista_t)):
  file3.write('%.6f\t%.6f\n' % (lista_x[i], lista_IB[i]))
file3.close()
'''
file4 = open('distribution.txt', 'w')
file4.write('###########################################################################################\n')
file4.write('distribuições\n')
file4.write('###########################################################################################\n\n')  
for i in range(len(L1_soma)-1):
    file4.write('%i;%i;%i;%i;%i\n' % (abc[i], L1_soma[i], L2_soma[i], L3_soma[i], L_soma[i]))

file4.close()

plt.figure(1)
plt.plot(abc, L1_soma, 'ko', ms=3)

plt.figure(2)
plt.plot(abc, L2_soma, 'rs', ms=3)

plt.figure(3)
plt.plot(abc, L3_soma, 'b*', ms=3)
