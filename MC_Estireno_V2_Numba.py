...
#################################################################################################
@jit(nopython=True)
def MC(x, xf, t, X, f, Reac, M0, ktc0MC, kpMC, kd1MC, kd2MC, ki1MC, ki2MC, kfmMC, kthMC):
  ...
  while x < xf:
    ...
    t += np.log(1 / np.random.rand()) / np.sum(Reac)
    
    R = np.random.rand() * np.sum(Reac)
    ...
    if R <= Reac[0]:
      ...
      if r4 > f:
        L1.append(int(P1[X[4] - 1]));P1.pop(X[4] - 1)
        X[1] -= 1
        X[4] -= 1
        X[6] += 1
...
L1, L2, L3, t, x = MC(x, xf, t, X, f, Reac, M0, ktc0MC, kpMC, kd1MC, kd2MC, ki1MC, ki2MC,kfmMC, kthMC)
