...
V, n_loops = 1.e-17, np.array([1, 5, 10, 50, 75, 100, 250, 500, 750], dtype=int)
start = time.time()
...
for k in range(len(n_loops)):
  ...
  for rep in range(n_loops[k]):
    ...
    L1, L2, L3 = MC_Estireno(x, xf, t, X, f, Reac, M0, ktc0MC, kd1MC, kd2MC, kfmMC, ki1MC, ki2MC, kpMC, kthMC)
    L1_soma += L1; L2_soma += L2; L3_soma += L3
  ...
  filename = "%d_resultados_Numba.txt" % k
